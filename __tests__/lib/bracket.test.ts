 ...
 */
import { generateSingleElimination } from '@/lib/brackets/single'

describe('generateSingleElimination', () => {
  it('generates 15 matches for 16 players', () => {
    const players = Array.from({ length: 16 }, (_, i) => `user-${i}`)
    const matches = generateSingleElimination('t-1', players)
  ...
  it('throws for non-power-of-2 player count', () => {
    const players = Array.from({ length: 3 }, (_, i) => `user-${i}`)
    expect(() => generateSingleElimination('t-1', players)).toThrow()
  })
